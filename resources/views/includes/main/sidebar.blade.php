    ...
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand" style="padding-left: 15px">
            <a href="#" style="display: flex; align-items: center;">
                <img src="{{ asset('theme/img/logo_fakultas_psi.png') }}" width="40" alt="Logo"
        ...
        <ul class="sidebar-menu">
            @if (Auth::user()->role == 'USER')
                ...
                <li class="{{ request()->is('calendar*') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('user.calendar.index') }}">
                        <i class="fas fa-calendar-alt"></i> <span>Kalender Ruangan</span>
                    </a>
                </li>
            ...
            @if (Auth::user()->role == 'ADMIN')
                ...
                {{-- TAMBAHKAN MENU KALENDER UNTUK ADMIN --}}
                <li class="menu-header">KALENDER</li>
                ...
                <li class="{{ request()->is('admin/info-image*') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('admin.info-image.index') }}">
                        <i class="fas fa-images"></i> <span>Info Gambar</span>
                    </a>
                </li>
                ...
                <li class="{{ request()->is('admin/booking-list*') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('booking-list.index') }}">
                        ...
                        <span>{{ $booking_list->where('status', 'PENDING')->count() > 0 ? '(' . $booking_list->where('status', 'PENDING')->count() . ')' : '' }}
                            Booking List</span>
                    </a>
                </li>
                <li class="{{ request()->is('admin/alat-test-booking-list*') ? 'active' : '' }}">
                    <a class="nav-link" href="{{ route('alat-test-booking-list.index') }}">
                        ...
                        <i class="fas fa-list"></i>
                        <span>{{ $tool_booking_list->where('status', '!=', 'tersedia')->count() > 0 ? '(' . $tool_booking_list->where('status', '!=', 'tersedia')->count() . ')' : '' }}
                            Alat test Booking List</span>
                    </a>
                </li>
